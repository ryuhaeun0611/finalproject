...
- **숲가꾸기/조림 밀도**: 산림청 『산림임업통계연보』 및 숲가꾸기 실적 보고서 기반 (2022)
- **수관 밀도**: 국립산림과학원 『임상도』 GIS 자료 기준 (1:5,000)
- **침엽수 비율**: 통계청 KOSIS 『임상별 산림면적 통계』 및 국립산림과학원 『국가산림자원조사 5차 보고서』
- **평균 경사도**: 국토지리정보원 『수치표고모델(DEM) 5m/30m』 및 관련 GIS 분석 결과
- **임분 밀도**: 국립산림과학원 『전국 산림자원조사』에 따른 ha당 입목 수 평균치 정규화
- **상대습도**: 기상청 『기후통계표』 및 국립기상과학원·산림과학원 『산불기상지수 및 기후변화 영향 연구』 (2023)
...
data = {
    "forest_care_density": np.random.uniform(0.10, 0.70, size=n),
    "canopy_density":      np.random.uniform(0.50, 0.95, size=n),
    "conifer_ratio":       np.random.uniform(0.30, 0.78, size=n),
    "slope_degree":        np.random.uniform(5, 45, size=n),
    "stand_density":       np.random.uniform(0.40, 0.82, size=n),
...
base = (
    60
    - 80 * df["forest_care_density"]
    + 70 * df["canopy_density"]
    + 100 * df["conifer_ratio"]
    + 2.5 * df["slope_degree"]
    + 70 * df["stand_density"]
    - 1.5 * df["humidity"]
)
noise = np.random.normal(0, 15, size=n)
df["fire_damage_area"] = np.round(np.clip(base + noise, 0, None), 1)
...
st.caption("※ 하층습기 지수 = `습도 × (1 − 숲가꾸기 밀도)`로 단순화한 지표입니다. ")
